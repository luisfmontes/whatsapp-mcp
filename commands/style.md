...
Tell the user it's saved and will apply automatically to future `send_message`/drafted messages — no need to re-explain their style each time. Mention they can re-run `/whatsapp-mcp-win:style` later if their writing changes or the profile feels off, and the old memory will be updated (not duplicated — check for the existing memory file first per the "don't write duplicate memories" rule).
