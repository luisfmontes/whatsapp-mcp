...
- `get_group_info` describes members by **name** plus an opaque `ref`, never a phone number, JID or LID — a 40-person group used to answer with 40 phone numbers. To act on a member, hand that `ref` back to `update_group_participants` as `"ref:<token>"`; it is tied to the group that minted it and lasts 10 minutes. Adding someone who is not in the group yet still takes a number, because there is nothing else to address them by.
