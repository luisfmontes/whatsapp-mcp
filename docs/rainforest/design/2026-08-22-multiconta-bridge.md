...
- **D3 — a conta se identifica por apelido (`pessoal`, `trabalho`); o campo `jid` do mapa é reservado e não é preenchido** — porquê: o JID já mudou de forma uma vez com o LID, e número não se lê num log, então o endereçamento é sempre por apelido. A versão original desta decisão dizia que o JID seria gravado no mapa quando o QR fechasse; a revisão de 2026-08-23 mostrou que isso nunca foi implementado e que **ninguém consome o campo** — nem o instalador, nem o servidor MCP, nem o bridge. Emendada em vez de implementada: construir um caminho de escrita que nada lê seria trabalho para satisfazer uma frase deste documento. O campo continua no schema para que preenchê-lo um dia não seja mudança de formato.
