...
- **D6 — a D2 abriu uma regressão, e ela se fecha nas DUAS pontas: `get_sender_name` captura `ValueError` e cai no fallback `sender_jid`, e `format_message` para de engolir exceção** — porquê: achado na revisão de 2026-08-23 e reproduzido por execução. Com a D2 no ar, um 500 no `/sender_name` levanta, o `except Exception` largo do `format_message` (`whatsapp.py:287`) engole, e a linha que concatena `From:` e o conteúdo nunca roda — a mensagem **desaparece inteira** da saída do `list_messages`. Antes: `'[...] Chat: Alice From: 5562...: texto que o usuario precisa ler'`. Depois: `'[...] Chat: Alice '`. É a mesma classe de defeito que esta entrega existe para fechar, reintroduzida por ela.
  ...
  Raio de alcance medido: `_api_post` tem **9 chamadores**, e **todos os 9** tratam `None` explicitamente — `get_sender_name`, `list_messages`, `get_message_context`, `list_chats`, `search_contacts`, `get_contact_chats`, `get_last_interaction`, `get_chat`, `get_direct_chat_by_contact`.

  Só `get_sender_name` é perigoso, e o critério **não é ter fallback de `None`** — é ser chamado por algo que engole exceção. `format_message` engolia; `get_message_context` faz o certo e superfície; os outros sete são funções de tool, onde a exceção subir até a borda é o comportamento desejado.

  *Números corrigidos em 2026-08-23, na revisão da rodada 2.* A primeira versão desta linha dizia "10 chamadores, 5 com fallback": o 10 contava a própria `def _api_post`, e o 5 saiu de um `grep -A 3` que não alcançava os chamadores cujo payload ocupa mais de três linhas. O revisor pegou que a conta não fechava e propôs "9 e 8"; conferindo na mão, é **9 e 9**. A conclusão não muda — muda a razão pela qual ela vale, e era a razão que estava escrita errada.
